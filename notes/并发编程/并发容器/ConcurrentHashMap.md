...
![image-20231011160536836](C:\Users\69425\AppData\Roaming\Typora\typora-user-images\image-20231011160536836.png)

Java 7 中 `ConcurrentHashMap` 的存储结构如上图，`ConcurrnetHashMap` 由很多个 `Segment` 组合，而每一个 `Segment` 是一个类似于 `HashMap` 的结构，所以每一个 `HashMap` 的内部可以进行扩容。但是 `Segment` 的个数一旦**初始化就不能改变**，默认 `Segment` 的个数是 16 个，你也可以认为 `ConcurrentHashMap` 默认支持最多 16 个线程并发。

> 个人理解：将一个map分割为N块，最多允许N个线程分别访问每块的数据
>
...
![image-20231011161137885](C:\Users\69425\AppData\Roaming\Typora\typora-user-images\image-20231011161137885.png)
...
```java
    ...
    private final Node<K,V>[] initTable() {
        ...
        while ((tab = table) == null || tab.length == 0) {
            if ((sc = sizeCtl) < 0)	//正在初始化或者正在扩容
                //使当前线程从执行状态变为可执行状态（就绪状态）即自旋
                Thread.yield(); // lost initialization race; just spin
            else if (U.compareAndSwapInt(this, SIZECTL, sc, -1)) {	//sc没变置为-1返回true,否则直接false
                try {
...
Java8 中的 `ConcurrentHashMap` 使用的 `Synchronized` 锁加 CAS 的机制。结构也由 Java7 中的 **`Segment` 数组 + `HashEntry` 数组 + 链表** 进化成了 **Node 数组 + 链表 / 红黑树**，Node 是类似于一个 HashEntry 的结构。它的冲突在达到一定大小时会转化成红黑树，在冲突小于一定数量时又退回链表。
