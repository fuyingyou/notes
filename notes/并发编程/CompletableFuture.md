...
`CompletionStage` 接口中的方法比较多，`CompletableFuture` 的函数式能力就是这个接口赋予的。其大量使用了 Java8 引入的函数式编程。
...
`supplyAsync()` 方法接受的参数是 `Supplier<T>` ，这也是一个函数式接口，`T` 是返回结果值的类型。当你需要异步操作且关心返回结果的时候,可以使用 `supplyAsync()` 方法。
...
![image-20231015211855815](C:\Users\69425\AppData\Roaming\Typora\typora-user-images\image-20231015211855815.png)
